...
--	SELECT @MedalRate=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'MedalRate'

	-- 银行税率
--	DECLARE @RevenueRate AS INT
--	SELECT @RevenueRate=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'RevenueRate'
...
--	BEGIN
--		SELECT @WinExperience=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'WinExperience'	
--	END
...
--	IF @WinExperience IS NULL SET @WinExperience=0
	...
	FROM WHJHPlatformDBLink.WHJHPlatformDB.dbo.GameGameItem TableGame,WHJHPlatformDBLink.WHJHPlatformDB.dbo.GameKindItem TableKind
	WHERE TableGame.GameID=TableKind.GameID	AND TableKind.KindID=@wKindID

	-- 兑换比率
--	DECLARE @PresentExchangeRate AS INT
--	SELECT @PresentExchangeRate=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'PresentExchangeRate'

	-- 兑换比率
--	DECLARE @RateGold AS INT
--	SELECT @RateGold=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'RateGold'
...
--		@ClientVersion AS ClientVersion, @ServerVersion AS ServerVersion, @PresentExchangeRate AS PresentExchangeRate, @RateGold AS RateGold
	SELECT @ClientVersion AS ClientVersion, @ServerVersion AS ServerVersion
END
