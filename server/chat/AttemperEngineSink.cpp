...
{
	ALDeb("Client cmd %d-%d", Command.wMainCmdID, Command.wSubCmdID);
	switch (Command.wMainCmdID)
	{
...
bool CAttemperEngineSink::OnTCPNetworkSubMBLogonByUserID(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	ALStd("%d:%d user logon %p", dwSocketID, pLogonUserID->dwUserID, pServerUserItem);
	if(pServerUserItem!=NULL)
	{
		//本地用户
...
bool CAttemperEngineSink::OnTCPNetworkSubTrumpet(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	ALDeb("%d:%d in useritem %p", dwSocketID, pUserChat->dwSendUserID, pServerUserItem);
	...
	if(pServerUserItem==NULL ) {
		...
		WORD wHeadSize=sizeof(TrumpetEcho)-sizeof(TrumpetEcho.szDescribeString);
		SendData(dwSocketID,MDM_GC_USER,SUB_GC_TRUMPET_ECHO,&TrumpetEcho,wHeadSize);
	}
	// remove post DBR_GR_TRUMPET with struct DBR_GR_Trumpet
	// directory send trumpet
  ...
  ALDeb("%d:%d send trumpet", dwSocketID, pUserChat->dwSendUserID);
	return true;
}
