...
BEGIN
	...
	-- 查询背包
	SELECT @GoodsCount = GoodsCount FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsPackage WHERE UserID=@dwUserID and GoodsID=@PropID
	IF @GoodsCount = 0 
	BEGIN
	...
	if @CurGoodsCount > 0
		update WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsPackage set GoodsCount = @CurGoodsCount where UserID=@dwUserID and GoodsID=@PropID
	else 
		delete WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsPackage where UserID=@dwUserID and GoodsID=@PropID
	
		
	--道具记录
	insert WHJHRecordDBLink.WHJHRecordDB.dbo.RecordUseProperty(SourceUserID, TargetUserID, 
