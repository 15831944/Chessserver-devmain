	...
	case ServiceStatus_Service:	//服务状态
		{
			...
			if (uvpp_init()) {
				wchar_t wchannel[32] = {0};
				CFG_READ_STR(COMMON,RPC,"127.0.0.1:7002",wchannel);
				UNI2UTF_S(wchannel, pChan);
				ALErr("Game rpc channel:%s", pChan);
				RPCGAME(rpc);
				rpc.Connect(pChan);
			}
			break;
...
VOID CGameServerDlg::UpdateParameterStatus(tagModuleInitParameter & ModuleInitParameter)
{
	...
	UNI2UTF_S(szString, pStr);
	ALErr("%s", pStr);
	
	TCHAR title[128];
	_sntprintf_s(title, CountArray(title), L"运行PID:%d", GetCurrentProcessId());
	CTraceService::TraceString(title,TraceLevel_Normal);
...
bool CGameServerDlg::StartServerService(WORD wServerID)
{
	...
	{
		const TCHAR* pszDescribe = TEXT("房间配置参数不存在或者加载失败");
...
BOOL CGameServerDlg::OnQueryEndSession()
{
	...
	if (m_ServiceUnits.GetServiceStatus()!=ServiceStatus_Stop)
	{
		const TCHAR* pszDescribe = TEXT("服务正在运行中，系统要求注销回话请求失败");
