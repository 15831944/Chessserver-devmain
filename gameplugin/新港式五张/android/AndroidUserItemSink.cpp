	...
	case IDI_USER_ADD_SCORE:	//加注定时
		{
			...
			//有效加注标识
			bool bValidAddScore = true;

			//加注效验
			if ((lAddScore + m_lUserScore[m_pIAndroidUserItem->GetChairID()] + m_lTableScore[m_pIAndroidUserItem->GetChairID()]) > m_lTurnMaxScore) 
			{
				bValidAddScore = false;
			}
			
			if ((lAddScore<0L)||((lAddScore + m_lUserScore[m_pIAndroidUserItem->GetChairID()] + m_lTableScore[m_pIAndroidUserItem->GetChairID()]) < m_lTurnLessScore)) 
			{
				bValidAddScore = false;
			}
...
bool CAndroidUserItemSink::OnSubGameEnd(const void * pBuffer, WORD wDataSize)
{
		...
		|| m_pIAndroidUserItem->GetMeUserItem()->GetUserScore() > m_lRobotScoreMax )
	{
		...
		//判断存取
		if (lRobotScore > m_lRobotScoreMax)
		{
			LONGLONG lSaveScore=0L;

			lSaveScore = LONGLONG(lRobotScore*m_lRobotBankSave/100);
		...
		else if (lRobotScore < m_lRobotScoreMin)
		{
			SCORE lScore = rand () % ((m_lRobotBankTake[1] - m_lRobotBankTake[0])) + m_lRobotBankTake[0];
			if (lScore > 0)
	...
	//如果取款未满足坐下最低成绩则取款
	if (m_pIAndroidUserItem->GetMeUserItem()->GetUserScore() <= pGameEnd->lMinTableScore)
	{
		m_pIAndroidUserItem->PerformTakeScore((pGameEnd->lMinTableScore-m_pIAndroidUserItem->GetMeUserItem()->GetUserScore()) * 1.5);		
	}
...
bool CAndroidUserItemSink::OnSubTrueEnd(const void * pBuffer, WORD wDataSize)
{
	...
	if( m_pIAndroidUserItem->GetMeUserItem()->GetUserScore() < m_lRobotScoreMin
		|| m_pIAndroidUserItem->GetMeUserItem()->GetUserScore() > m_lRobotScoreMax )
	{
		m_pIAndroidUserItem->SetGameTimer(IDI_BANK_OPERATION, (rand()%2) + 1);
	}
