...
BEGIN	
	...
	SET @ReturnValue=0
	SELECT @UserRight=UserRight FROM WHJHAccountsDB..AccountsInfo WHERE UserID=@dwUserID 
	SELECT @UserRight=@UserRight|UserRight FROM GameScoreInfo WHERE UserID=@dwUserID

	IF EXISTS(SELECT * FROM WHJHTreasureDB..GameScoreInfo WHERE UserID=@dwUserID)
	BEGIN
		DECLARE @Score BIGINT
		SELECT @Score=Score FROM WHJHTreasureDB..GameScoreInfo WHERE UserID=@dwUserID
		IF @Score < @dwMatchFee
		BEGIN
			SET @ReturnValue=1
		END
		ELSE
		BEGIN
			UPDATE WHJHTreasureDB..GameScoreInfo SET Score=Score-@dwMatchFee WHERE UserID=@dwUserID
