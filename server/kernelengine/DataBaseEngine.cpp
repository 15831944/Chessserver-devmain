...
//////////////////////////////////////////////////////////////////////////////////
#define OPENTIMEOUT 3
#define EXECTIMEOUT 5
time_t CDataBase::m_ReConnectedTime = 0;
...
VOID CDataBaseException::SetExceptionInfo(enSQLException SQLException, CComError * pComError)
{
    ...
    // 设置信息
    LPCTSTR pDesc = pComError->Description();
	LPCTSTR pError = pComError->ErrorMessage();
    m_SQLException = SQLException;
    m_hResult = pComError->Error();
    m_strDescribe.Format(TEXT("数据库异常:[0x%8x]:%s:%s"), m_hResult, pDesc ? pDesc:TEXT("???"), pError ? pError:TEXT("!!!"));
...
CDataBase::CDataBase() : m_dwResumeConnectCount(10L), m_dwResumeConnectTime(30L)
{
    ...
    m_DBCommand->CommandTimeout = EXECTIMEOUT;
    m_DBConnection->ConnectionTimeout = OPENTIMEOUT;
...
VOID CDataBase::ExecuteProcess(LPCTSTR pszSPName, bool bRecordset)
{
  ...
  do {
    try {
      ...
      _PLOG(KERNELLOGHOOK, "[%s]start.", pSP);
      if (bRecordset == true) {
      ...
      _PLOG(KERNELLOGHOOK, "[%s]end.", pSP);
      return ;
    } catch (CComError & ComError) {
      if (!IsConnectError()) {
        _PLOG(KERNELLOGHOOK, "[%s]end.ESyntax!", pSP);
        OnSQLException(SQLException_Syntax, &ComError);
      }
      _PLOG(KERNELLOGHOOK, "[%s]end.EConnection.%d", pSP, again);
      // try to connect once if connection error
      if ((++again) > 1 || !ReConnect()) {
        OnSQLException(SQLException_Connect, &ComError);
      }
      // connected
      _PLOG(KERNELLOGHOOK, "[%s]Connected.%d!", pSP, again);
    }
  } while (again < 2);
}
...
bool CDataBase::ReConnect() {
  ...
  if ((m_ReConnectedTime+OPENTIMEOUT) >= timeNow || (lastTime+10) <= timeNow ) {
    lastTime = timeNow;
    OpenConnection();
    m_ReConnectedTime = timeNow;
    g_TraceServiceManager.TraceString(TEXT("成功重新连接数据库"), TraceLevel_Warning);
    return true;
  }
  return false;
