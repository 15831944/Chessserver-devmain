...
BEGIN
	...
	WHERE UserID IN (
		SELECT TOP (@dwAndroidCountMember0) UserID FROM AndroidLockInfo(NOLOCK)
		WHERE AndroidStatus=0 AND ServerID=0 AND MemberOrder=0 AND DATEDIFF(hh,LockDateTime,GetDate())>=12
		ORDER BY NEWID()
	)
	-- 查询机器
	INSERT INTO #TempAndroids(UserID,LogonPass) SELECT UserID,LogonPass FROM AndroidLockInfo(NOLOCK)
	WHERE UserID IN (
		SELECT TOP (@dwAndroidCountMember1) UserID FROM AndroidLockInfo(NOLOCK)
		WHERE AndroidStatus=0 AND ServerID=0 AND MemberOrder=1 AND DATEDIFF(hh,LockDateTime,GetDate())>=12
		ORDER BY NEWID()
	)
	-- 查询机器
	INSERT INTO #TempAndroids(UserID,LogonPass) SELECT UserID,LogonPass FROM AndroidLockInfo(NOLOCK)
	WHERE UserID IN (
		SELECT TOP (@dwAndroidCountMember2) UserID FROM AndroidLockInfo(NOLOCK)
		WHERE AndroidStatus=0 AND ServerID=0 AND MemberOrder=2 AND DATEDIFF(hh,LockDateTime,GetDate())>=12
		ORDER BY NEWID()
	)
	-- 查询机器
	INSERT INTO #TempAndroids(UserID,LogonPass) SELECT UserID,LogonPass FROM AndroidLockInfo(NOLOCK)
	WHERE UserID IN (
		SELECT TOP (@dwAndroidCountMember3) UserID FROM AndroidLockInfo(NOLOCK)
		WHERE AndroidStatus=0 AND ServerID=0 AND MemberOrder=3 AND DATEDIFF(hh,LockDateTime,GetDate())>=12
		ORDER BY NEWID()
	)
	-- 查询机器
	INSERT INTO #TempAndroids(UserID,LogonPass) SELECT UserID,LogonPass FROM AndroidLockInfo(NOLOCK)
	WHERE UserID IN (
		SELECT TOP (@dwAndroidCountMember4) UserID FROM AndroidLockInfo(NOLOCK)
		WHERE AndroidStatus=0 AND ServerID=0 AND MemberOrder=4 AND DATEDIFF(hh,LockDateTime,GetDate())>=12
		ORDER BY NEWID()
	)
	-- 查询机器
	INSERT INTO #TempAndroids(UserID,LogonPass) SELECT UserID,LogonPass FROM AndroidLockInfo(NOLOCK)
	WHERE UserID IN (
		SELECT TOP (@dwAndroidCountMember5) UserID FROM AndroidLockInfo(NOLOCK)
		WHERE AndroidStatus=0 AND ServerID=0 AND MemberOrder=5 AND DATEDIFF(hh,LockDateTime,GetDate())>=12
		ORDER BY NEWID()
	)
	-- 更新状态
