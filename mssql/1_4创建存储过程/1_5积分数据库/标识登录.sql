...
BEGIN
	...
	-- 系统暂停
	SELECT @EnjoinLogon=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'EnjoinLogon'
	IF @EnjoinLogon IS NOT NULL AND @EnjoinLogon<>0
	BEGIN
		SELECT @strErrorDescribe=StatusString FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'EnjoinLogon'
		RETURN 2
	END
    ...
    FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.IndividualDatum WHERE UserID=@dwUserID
	...
	BEGIN
		...
		UPDATE WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo SET MoorMachine=@MoorMachine, LastLogonMachine=@strMachineID WHERE UserID=@UserID
	END
	...
	VALUES (@UserID, 0, @wKindID, @wServerID, @strClientIP, @strMachineID)
	...
	SELECT @Insure=InsureScore FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.GameScoreInfo WHERE UserID=@dwUserID
...
BEGIN
	...
	BEGIN
		-- 锁定解除
		DELETE WHJHTreasureDBLink.WHJHTreasureDB.dbo.GameScoreLocker WHERE UserID=@dwUserID --AND ServerID=@wServerID
		
		-- 查询锁定
		DECLARE @LockKindID INT
		DECLARE @LockServerID INT
		SELECT @LockKindID=KindID, @LockServerID=ServerID FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.GameScoreLocker WHERE UserID=@dwUserID --AND ServerID=@wServerID
	...
	-- 查询钻石
	SELECT @Dimand=Diamond FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.UserCurrency WHERE UserID=@UserID		
...
-- 进入记录
	...
	BEGIN
		...
		BEGIN
			...
			DELETE WHJHTreasureDBLink.WHJHTreasureDB.dbo.GameScoreLocker WHERE UserID=@dwUserID --AND ServerID=@wServerID
			INSERT WHJHTreasureDBLink.WHJHTreasureDB.dbo.GameScoreLocker (UserID, ServerID, KindID, EnterID, EnterIP, EnterMachine) VALUES (@dwUserID, @wServerID, @wKindID, @InoutIndex, @strClientIP, @strMachineID)
	...
	SELECT @UserID AS UserID, @GameID AS GameID, @GroupID AS GroupID, @NickName AS NickName, @UnderWrite AS UnderWrite,  @FaceID AS FaceID, @Dimand AS Dimand,
		@CustomID AS CustomID, @Gender AS Gender, @GroupName AS GroupName, @MasterOrder AS MemberOrder, @UserRight AS UserRight, @MasterRight AS MasterRight,
		@MasterOrder AS MasterOrder, @MemberOrder AS MemberOrder, 0 AS Score, @InoutIndex AS InoutIndex,
		0 AS Insure,  @WinCount AS WinCount,  @LostCount AS LostCount,@DrawCount AS DrawCount, @FleeCount AS FleeCount, 
		@IntegralCount AS IntegralCount,@AgentID AS AgentID,@QQ AS QQ, @EMail AS EMail, @SeatPhone AS SeatPhone, @MobilePhone AS MobilePhone,
		@DwellingPlace AS DwellingPlace,@PostalCode AS PostalCode
END
