...
BEGIN
  ...
  SELECT [ID]
      ...
      ,[RegulationsInfo]
      ,[SortID]
      ,[Name] FROM GameProperty(NOLOCK) WHERE Nullity=0 AND Kind <> 11
END
...
BEGIN
	...
	-- 初始信息
	SET	@iProPertyCount= 0
	SET @ExchangeRatio = 0

	-- 道具判断
	SELECT @PropertyName=Name, @ExchangeRatio=ExchangeRatio	FROM GameProperty(NOLOCK) WHERE Nullity=0 AND ID=@dwPropertyID
	...
	-- 道具计算
	SET @iProPertyCount=@iDiamondCount*@ExchangeRatio
	
	-- 读取银行
	DECLARE @TotalDiamond BIGINT
	SELECT @TotalDiamond=Diamond FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.UserCurrency WHERE UserID=@dwUserID

	-- 数量判断
	IF @TotalDiamond<@iDiamondCount OR @TotalDiamond IS NULL
	BEGIN
		-- 错误信息
	...
	END
	-- 扣掉钻石	
	UPDATE WHJHTreasureDBLink.WHJHTreasureDB.dbo.UserCurrency SET Diamond=Diamond-@iDiamondCount WHERE UserID=@dwUserID
	
	--加入背包
	UPDATE WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsPackage SET GoodShowID=0, GoodsCount=GoodsCount+ @iProPertyCount, PushTime=GETDATE() WHERE UserID=@dwUserID AND GoodsID=@dwPropertyID
	IF @@ROWCOUNT = 0
	BEGIN
		INSERT WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsPackage(UserID,GoodsID,GoodShowID,GoodsCount,PushTime)	VALUES(@dwUserID, @dwPropertyID,0,@iProPertyCount,GETDATE())
	END
	--END
	...
	VALUES(@dwUserID,@dwPropertyID,@PropertyName,@ExchangeRatio,@iProPertyCount,@TotalDiamond,@iDiamondCount,@strClientIP,GETDATE())
...
--	SELECT @TotalDiamond=Diamond FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.UserCurrency WHERE UserID=@dwUserID
	...
	VALUES(@SerialNumber,0,@dwUserID,9,@TotalDiamond,-@iDiamondCount,@strClientIP,@DateTime)
	
	IF @TotalDiamond IS NULL SET @TotalDiamond=0
	SET @strErrorDescribe=N'恭喜您！购买' + @PropertyName + N' ×' + CAST(@iProPertyCount AS NVARCHAR) +N' ' + N'成功！'
	
	--输出记录
	SELECT @dwPropertyID AS PropertyID,@iProPertyCount AS ItemCount,@TotalDiamond-@iDiamondCount  as Diamond
