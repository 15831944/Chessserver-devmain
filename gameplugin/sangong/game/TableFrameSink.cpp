...
bool CTableFrameSink::Initialization(IUnknownEx *pIUnknownEx)
{
    ...
    //房卡配置
    m_dwBaseScore	= m_pGameCustomRule->dwBaseScore;
...
bool CTableFrameSink::OnActionUserSitDown(WORD wChairID, IServerUserItem *pIServerUserItem, bool bLookonUser)
{
    ...
    if((m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) != 0) //房卡模式
    {
        ...
        if(pSetInfo[0] == 1)
        {
            ...
            m_dwBaseScore	= pSetInfo[3];			//底分
            m_bCallBanker	= pSetInfo[4];			//是否抢庄

            lyt.Format("lytlog:: 用户坐下-------游戏人数= %d 底分= %d 是否抢庄= %d  ", m_wPlayerCount, m_dwBaseScore, m_bCallBanker);
            OutputDebugStringA(lyt);
        }
    }
...
bool CTableFrameSink::OnEventGameStart()
{
    ...
    m_pITableFrame->SendGameMessage(strInfo, SMT_GAMELOG);
    ...
    for(int i = 0; i < m_wPlayerCount; i++)
    {
        ...
        else
        {
            ...
            strInfo.Format(TEXT("玩家%d 状态：%d"), i, pIServerUser->GetUserStatus());
            m_pITableFrame->SendGameMessage(strInfo, SMT_GAMELOG);
        }
    ...
    else	//9点上庄
    {
        ...
        for(int i = 0; i < m_wPlayerCount; i++)
        {
            ...
            {
                strInfo.Format(TEXT("用户昵称 %s----玩家坐位号：%d，当前庄家：%d"), pUserItem->GetNickName(), pUserItem->GetChairID(), m_wBankerUser);
                m_pITableFrame->SendGameMessage(strInfo, SMT_GAMELOG);
            }
        }


        strInfo.Format(TEXT("当前庄家：%d 昵称 %s 坐位号：%d"), m_wBankerUser, m_pITableFrame->GetTableUserItem(m_wBankerUser)->GetNickName(), m_pITableFrame->GetTableUserItem(m_wBankerUser)->GetChairID());
        m_pITableFrame->SendGameMessage(strInfo, SMT_GAMELOG);

        //设置变量
        CMD_S_AddBegin AddBegin;
        AddBegin.wBankerUser = m_wBankerUser;
        AddBegin.lCellScore = m_dwBaseScore;
...
bool CTableFrameSink::OnEventGameConclude(WORD wChairID, IServerUserItem *pIServerUserItem, BYTE cbReason)
{
    ...
    case GER_NORMAL:		//常规结束
    {
        ...
        for(WORD i = 0; i < m_wPlayerCount; i++)
        {
            ...
            //房卡模式
            if(((m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) != 0) && (m_stRecord.nCount < MAX_RECORD_COUNT))
            {
        ...
        for(WORD y = 0; y < m_wPlayerCount; y++)
        {
            ...
            if(pICallUser != NULL)
            {
                if(pICallUser->GetUserScore() < m_dwBaseScore * 40)
                {
        ...
        for (WORD i=0; i<GAME_PLAYER; i++) {
            ...
            if (GameEnd.lGameScore[i] > 0) {
                ...
                sTrumpet.dwCardType = GameEnd.cbType[i];
...
bool CTableFrameSink::OnEventSendGameScene(WORD wChiarID, IServerUserItem *pIServerUserItem, BYTE cbGameStatus, bool bSendSecret)
{
    ...
    case GAME_STATUS_FREE:		//空闲状态
    {
        ...
        //规则
        StatusFree.dwBaseScore	= m_dwBaseScore;
    ...
    case GS_TK_CALL:	//叫庄状态
    {
        ...
        //规则
        StatusCall.dwBaseScore	= m_dwBaseScore;
    ...
    case GS_TK_SCORE:	//下注状态
    {
        ...
        //规则
        StatusScore.dwBaseScore	= m_dwBaseScore;
    ...
    case GS_TK_PLAYING:	//游戏状态
    {
        ...
        //规则
        StatusPlay.dwBaseScore	= m_dwBaseScore;
...
bool CTableFrameSink::OnUserAddScore(WORD wChairID, LONGLONG lScore)
{
    ...
    {
        ASSERT(lScore > 0 && lScore <= m_dwBaseScore * 20);
        if(lScore <= 0 || lScore > m_dwBaseScore * 20)
        {
            return false;
        }
    }
...
void CTableFrameSink::OnBeginAddScore()
{
    ...
    CString strInfo;
    strInfo.Format(TEXT("开始下注 -----------当前庄家：%d"), m_wBankerUser);
    m_pITableFrame->SendGameMessage(strInfo, SMT_GAMELOG);
