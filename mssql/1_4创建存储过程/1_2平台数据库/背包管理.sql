...
-- 变量定义
	...
	if @dwKind = 0
		select * from WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsPackage as a, GameProperty as b where a.GoodsID=b.ID and a.UserID=@dwUserID and a.GoodsCount > 0
	else
		select * from WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsPackage as a, GameProperty as b where a.GoodsID=b.ID and a.UserID=@dwUserID and a.GoodsCount > 0 and b.Kind=@dwKind
	RETURN 0
	END
...
BEGIN
	...
	-- 查询背包
	SELECT @GoodsCount = GoodsCount FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsPackage WHERE UserID=@dwUserID and GoodsID=@dwPropID
	...
	BEGIN
		-- 查询玩家
		IF NOT EXISTS (SELECT UserID FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo WHERE @dwRecvUserID=UserID)
		BEGIN
	...
	BEGIN
		...
		UPDATE WHJHTreasureDBLink.WHJHTreasureDB.dbo.GameScoreInfo SET Score = @CurrGetGold + Score WHERE UserID=@dwUserID
	END
	...
	BEGIN
		...
		--插入Buff表
		SELECT @UseTime=UseTime, @ValidTime = UseResultsValidTime FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfoPoperty WHERE UserID=@dwUserID and KindID=@Kind
		IF @ValidTime = 0
		BEGIN
			SET @UseTime = GETDATE()
			SET @ValidTime = @UseResultsValidTime*@dwPropCount
			INSERT WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfoPoperty(UserID, KindID, UseTime, UseResultsValidTime, UseResultsValidTimeScoreMultiple, Name) 
		...
		BEGIN
			...
			BEGIN
				...
				UPDATE WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfoPoperty SET UseResultsValidTime=@ValidTime WHERE UserID=@dwUserID and KindID=@Kind
			END
			ELSE	--过期
			BEGIN
				SET @UseTime = GETDATE()
				SET @ValidTime = @UseResultsValidTime*@dwPropCount
				UPDATE WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfoPoperty SET UseTime=@UseTime, UseResultsValidTime=@ValidTime WHERE UserID=@dwUserID and KindID=@Kind
			END
		END
	END
	...
	BEGIN
		...
		-- 删除过期
		DELETE FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsMember WHERE UserID=@dwUserID  AND MemberOverDate<=GETDATE()
		
		-- 查询权限	
		SELECT @MemberRight=UserRight FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.MemberProperty
		...
		-- 会员记录
		UPDATE WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsMember SET MemberOverDate=DATEADD(mm,@MemberValidMonth, MemberOverDate)
		WHERE UserID=@dwUserID AND MemberOrder=@MemberOrder
		IF @@ROWCOUNT=0
		BEGIN
		INSERT WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsMember(UserID,MemberOrder,UserRight,MemberOverDate)
		...
		FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsMember WHERE UserID=@dwUserID
		
		-- 切换权限
		SELECT @MaxUserRight=UserRight FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsMember
		WHERE UserID=@dwUserID AND MemberOrder=@MaxMemberOrder	
		
		-- 更新会员
		UPDATE WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo
	...
	BEGIN
		...
		SET @UserGameScore = 0
		SELECT @UserGameScore=Score FROM WHJHGameScoreDBLink.WHJHGameScoreDB.dbo.GameScoreInfo WHERE UserID=@dwUserID
		IF @UserGameScore >= 0
		BEGIN
		...
		END
		ELSE
			UPDATE WHJHGameScoreDBLink.WHJHGameScoreDB.dbo.GameScoreInfo SET Score=0 WHERE UserID=@dwUserID
	END
	...
	BEGIN
		...
		SET @FleeCount = 0
		SELECT @FleeCount=FleeCount FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.GameScoreInfo WHERE UserID=@dwUserID
		IF @FleeCount=0
		BEGIN
		...
		END
		ELSE
			UPDATE WHJHTreasureDBLink.WHJHTreasureDB.dbo.GameScoreInfo SET FleeCount=0 WHERE UserID=@dwUserID
	END
	...
	IF @CurGoodsCount > 0
	BEGIN
		UPDATE WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsPackage SET GoodsCount = @CurGoodsCount WHERE UserID=@dwUserID and GoodsID=@dwPropID
	END
	ELSE
	BEGIN
		DELETE WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsPackage WHERE UserID=@dwUserID and GoodsID=@dwPropID
	END
		
	--道具记录
	insert WHJHRecordDBLink.WHJHRecordDB.dbo.RecordUseProperty(SourceUserID, TargetUserID,PropertyID, PropertyName,
	...
	SET @Score = 0
	SELECT @Score=Score FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.GameScoreInfo WHERE UserID = @dwUserID 	
	
	--当前会员
	SELECT @CurrMemberOrder=MemberOrder FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo WHERE UserID = @dwUserID
