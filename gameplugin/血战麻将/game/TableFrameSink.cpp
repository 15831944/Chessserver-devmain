	...
	case GER_NORMAL:		//常规结束
		{
					...
					if (m_bPlayStatus[i] && m_ChiHuRight[i].IsEmpty())
					{
						...
						if (bHuaZhu)
						{
							...
							for (WORD j=0; j<GAME_PLAYER; j++)
							{
								...
								{
									lHuaZhuScore[i] -= m_pGameServiceOption->lCellScore * 64L;
									lHuaZhuScore[j] += m_pGameServiceOption->lCellScore * 64L;
									lHuaZhuScoreEx[i][j] = m_pGameServiceOption->lCellScore * 64L;
									lLostHuaZhuScore[j][i] = m_pGameServiceOption->lCellScore * 64L;
									
									m_tagHistoryChart[i][wChartIndex].bEffectChairID[j] = true;
									lChaHuaZhuScore += (-(m_pGameServiceOption->lCellScore * 64L));
									...
									m_tagHistoryChart[j][wHuaZhuChartIndex].lScore = m_pGameServiceOption->lCellScore * 64L;
							...
							if (!m_GameLogic.IsTingCard(m_cbCardIndex[i], m_WeaveItemArray[i], m_cbWeaveItemCount[i], m_cbCallCard[i]))
							{
								...
								for (WORD j=0; j<GAME_PLAYER; j++)
								{
									...
									if (m_GameLogic.IsTingCard(m_cbCardIndex[j], m_WeaveItemArray[j], m_cbWeaveItemCount[j], m_cbCallCard[j]))
									{
										...
										//查叫赔钱
										lChaJiaoScore[i] -= m_pGameServiceOption->lCellScore * wMaxFanShu;
										lChaJiaoScore[j] += m_pGameServiceOption->lCellScore * wMaxFanShu;
										lChaJiaoScoreEx[i][j] = m_pGameServiceOption->lCellScore * wMaxFanShu;
										lLostChaJiaoScore[j][i] = m_pGameServiceOption->lCellScore * wMaxFanShu;

										m_tagHistoryChart[i][wChartIndex].bEffectChairID[j] = true;
										lChaDaJiaoScore += (-(m_pGameServiceOption->lCellScore * wMaxFanShu));
										...
										m_tagHistoryChart[j][wDaJiaoChartIndex].lScore = m_pGameServiceOption->lCellScore * wMaxFanShu;
			...
			for( WORD i = 0; i < GAME_PLAYER; i++ )
			{
				...
				if (m_lGameScore[i] > 10000000)
				{
					...
					strOperationRecord.Format(TEXT("账户[%s],得分为 %I64d"), pUserItem->GetNickName(), m_lGameScore[i]);
					//写入日志
					WriteInfo(strOperationRecord, m_pGameServiceOption->szServerName);
				}
			}
			...
			for (WORD i=0; i<GAME_PLAYER; i++)
			{
				...
				for (WORD j=0; j<MAX_CHART_COUNT; j++)
				{
					if (m_tagHistoryChart[i][j].charttype != INVALID_CHARTTYPE)
					{
						strnickname.Format(TEXT("当前昵称[%s], 流水类型 %d 倍数 %I64d, 得分 %I64d GameEndscore = %I64d 作用玩家 ----"),
						...
						WriteInfo(strnickname, m_pGameServiceOption->szServerName);
					}
				}
			}
			...
			for (WORD i = 0; i<GAME_PLAYER; i++)
			{
				...
				if (GameEnd.lGameScore[i] > 0) {
					...
					sTrumpet.dwCardType = GameEnd.dwChiHuRight[i];
	...
	case GER_USER_LEAVE:		//用户强退
		{
			...
			for( WORD i = 0; i < GAME_PLAYER; i++ )
			{
				...
				if (m_bPlayStatus[i]) 
				{
					m_lGameScore[i] += 4L * m_pGameServiceOption->lCellScore;
					m_lGameScore[wChairID] -= 4L * m_pGameServiceOption->lCellScore;
				}
				m_lGameScore[i] += lGangScore[i];
			}
	...
	case GS_MJ_CHANGECARD:	//换三张
		{
			...
			StatusChangeCard.lCellScore = m_pGameServiceOption->lCellScore;
...
bool CTableFrameSink::OnTimerMessage(DWORD wTimerID, WPARAM wBindParam)
{
	if (wTimerID == IDI_OUT_CARD)
	{
		...
		OnUserOutCard(m_wCurrentUser, card);
		return true;
	}
			...
			if (pTrustee->bTrustee)
			{
				if (pIServerUserItem->GetChairID() == m_wCurrentUser)
				{
					...
					else
					{
						for(int i=0;i<MAX_INDEX;i++)
						{
							if(m_cbCardIndex[pIServerUserItem->GetChairID()][i]>0)
							{
								cardindex = i;
								break;
							}
					...
					if (m_cbCardIndex[pIServerUserItem->GetChairID()][cardindex] == 0)
					{
						//如果找不到牌就从后往前取牌
					...
					//如果轮到自己有操作，先过然后再出牌
					if (m_cbUserAction[pIServerUserItem->GetChairID()] != WIK_NULL)
					{
						OnUserOperateCard(pIServerUserItem->GetChairID(), WIK_NULL, 0);
					}

					OnUserOutCard(pIServerUserItem->GetChairID(), m_GameLogic.SwitchToCardData(cardindex));
				}
				else if (m_wCurrentUser == INVALID_CHAIR && m_cbUserAction[pIServerUserItem->GetChairID()] != WIK_NULL)
				{
					OnUserOperateCard(pIServerUserItem->GetChairID(), WIK_NULL, 0);
				}
			}
	...
	case SUB_C_CHANGE_CARD:	//换三张
		{
			...
			return OnUserChangeCard(pIServerUserItem->GetChairID(), pChangeCard->cbChangeCardData);
		}
...
bool CTableFrameSink::OnUserOperateCard(WORD wChairID, BYTE cbOperateCode, BYTE cbOperateCard)
{
	...
	if (m_wCurrentUser == INVALID_CHAIR)
	{
		...
		if (cbTargetAction == WIK_GANG)
		{
			...
			m_tagHistoryChart[wTargetUser][wTargetChartIndex].lScore = m_pGameServiceOption->lCellScore*2;
			...
			m_tagHistoryChart[m_wProvideUser][wProvideChartIndex].lScore = -m_pGameServiceOption->lCellScore*2;
		...
		case WIK_GANG:			//杠牌操作
			{
				...
				if (m_cbCardIndex[wChairID][cbCardIndex] == 1)
				{
					...
					m_tagHistoryChart[wChairID][wTargetChartIndex].lScore = cbValidCount * m_pGameServiceOption->lCellScore;
					...
					for (WORD i=0; i<GAME_PLAYER; i++)
					{
						...
						m_tagHistoryChart[i][wChartIndex].lTimes = 1;
						m_tagHistoryChart[i][wChartIndex].lScore = -m_pGameServiceOption->lCellScore;
				...
				else
				{
					...
					m_tagHistoryChart[wChairID][wTargetChartIndex].lScore = cbValidCount * m_pGameServiceOption->lCellScore * 2;
					...
					for (WORD i=0; i<GAME_PLAYER; i++)
					{
						...
						m_tagHistoryChart[i][wChartIndex].lScore = -m_pGameServiceOption->lCellScore * 2;
						m_tagHistoryChart[i][wChartIndex].bEffectChairID[wChairID] = true;
...
//用户换三张
bool CTableFrameSink::OnUserChangeCard(WORD wChairID, BYTE cbChangeCardData[MAX_CHANGE_CARDCOUNT])
{
	//校验是否已经换过
...
bool CTableFrameSink::DispatchCardData(WORD wCurrentUser, bool bTail)
{
	...
	if(m_bTrustee[wCurrentUser])
	{
		m_pITableFrame->SetGameTimer(IDI_OUT_CARD, 3000, 1, 0);
	}
