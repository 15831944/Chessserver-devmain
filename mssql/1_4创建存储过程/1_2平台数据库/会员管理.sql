...
BEGIN
	-- 加载会员
	SELECT * FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.MemberProperty
...
BEGIN
	...
	-- 查询用户
	IF not exists(SELECT * FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID AND LogonPass=@strPassword)
	BEGIN
		return 1
	END
    ...
    SET   @DayGiftID = 0
	SELECT @DayGiftID=DayGiftID FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.MemberProperty where @CurrMemberOrder =MemberOrder
...
BEGIN
    ...
    BEGIN
		...
		-- 送金记录
		SELECT @TakeGold=TakeGold FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsMemberDayPresent WHERE UserID=@dwUserID AND TakeDateID=@TodayDateID
		IF @TakeGold IS NULL 
		BEGIN
			SET @Present=1
		END
		...
		DECLARE @GiftID INT
		-- 礼包记录
		SELECT @GiftID=GiftID FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsMemberDayGift WHERE UserID=@dwUserID AND TakeDateID=@TodayDateID
		IF @GiftID IS NULL 
		BEGIN
			SET @Gift=1
		END
		...
		SELECT @CurrPresentGold =DayPresent,@CurrPresentGiftID= DayGiftID from WHJHAccountsDBLink.WHJHAccountsDB.dbo.MemberProperty WHERE @CurrMemberOrder = MemberOrder
...
BEGIN
    ...
    SET @lRewardGold = 0
    SELECT @lRewardGold = DayPresent FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.MemberProperty where @CurrMemberOrder =MemberOrder
    ...
    -- 领取金币    
    UPDATE  WHJHTreasureDBLink.WHJHTreasureDB.dbo.GameScoreInfo SET Score = Score + @lRewardGold WHERE UserID = @dwUserID
    
     -- 更新记录
    IF @Present=1
	BEGIN
		INSERT INTO WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsMemberDayPresent(UserID,TakeDateID,TakeGold) VALUES(@dwUserID,@TodayDateID,@lRewardGold)        
	END 
	...
	UPDATE WHJHTreasureDBLink.WHJHTreasureDB.dbo.StreamPresentInfo SET DateID=@DateID, PresentCount=PresentCount+1,PresentScore=PresentScore+@lRewardGold WHERE UserID=@dwUserID AND TypeID=9
	IF @@ROWCOUNT=0
	BEGIN
		INSERT WHJHTreasureDBLink.WHJHTreasureDB.dbo.StreamPresentInfo(DateID,UserID,TypeID,PresentCount,PresentScore) VALUES(@DateID,@dwUserID,9,1,@lRewardGold)
	END	
...
BEGIN
    ...
    ELSE
    BEGIN
		UPDATE WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsPackage SET GoodShowID=0, GoodsCount=GoodsCount+1, PushTime=GETDATE() WHERE UserID=@dwUserID AND GoodsID=@PROPERTYID
		IF @@ROWCOUNT =0
		BEGIN
			INSERT WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsPackage(UserID,GoodsID,GoodShowID,GoodsCount,PushTime)
    ...
    IF @Gift=1
	BEGIN
		INSERT INTO WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsMemberDayGift(UserID,TakeDateID,GiftID) VALUES(@dwUserID,@TodayDateID,@PROPERTYID)        
	END 
