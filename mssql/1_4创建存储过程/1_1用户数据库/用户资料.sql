...
--	BEGIN
--		INSERT INTO WHJHRecordDBLink.WHJHRecordDB.dbo.RecordAccountsExpend(UserID,ReAccounts,ClientIP)
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			SET @strErrorDescribe=N'绑定成功，服务器没有配置奖励！'
			RETURN 7
		END
		ELSE
		BEGIN
			DECLARE @CurrentDiamond INT
			SELECT @CurrentDiamond=Diamond FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.UserCurrency WHERE UserID=@dwUserID
			--更新钻石
			IF @CurrentDiamond IS NOT NULL
			BEGIN
				UPDATE WHJHTreasureDBLink.WHJHTreasureDB.dbo.UserCurrency SET Diamond=Diamond+@dwRewardDiamond WHERE UserID=@dwUserID
			END
			ELSE
			BEGIN
				SET @CurrentDiamond=0
				INSERT INTO WHJHTreasureDBLink.WHJHTreasureDB.dbo.UserCurrency(UserID,Diamond)
