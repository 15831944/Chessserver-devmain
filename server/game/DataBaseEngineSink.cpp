...
bool CDataBaseEngineSink::OnDataBaseEngineRequest(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBR_GR_WRITE_VIDEO_DATA_INFO:
		...
		break;
	case DBR_GR_CONFIG_PULSE: {
			bSucceed = OnRequestGameConfigPulse(dwContextID,pData,wDataSize,dwUserID);
		} break;
	}
	...
	try
	{
		...
		if (lResultCode==DB_SUCCESS)
		{
			...
			for (WORD i=0;i<pGameScoreRecord->wRecordCount;i++)
			{
				...
				if (pGameScoreRecord->GameScoreRecord[i].lScore > 0) {
					RPCGAME(rpc);
					rpc.GameSaleStatis(pGameScoreRecord->GameScoreRecord[i].dwUserID,
						pGameScoreRecord->GameScoreRecord[i].lScore+pGameScoreRecord->GameScoreRecord[i].lRevenue,
...
bool CDataBaseEngineSink::OnRequestLoadAndroidUser(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	...
	{		
		...
		ALStd("Loaded Android Count %d", GameAndroidInfo.wAndroidCount);
		return true;
	}
	catch (IDataBaseException * pIException)
	{
...
VOID CDataBaseEngineSink::OnLogonDisposeResult(DWORD dwContextID, DWORD dwErrorCode, LPCTSTR pszErrorString, bool bMobileClient,BYTE cbDeviceType,WORD wBehaviorFlags,WORD wPageTableCount)
{
	if (dwErrorCode==DB_SUCCESS)
	{
		...
		}catch(...)
		{
			...
			ALErr("DBException GSP_GR_QueryUserGameData");
		}
...
bool CDataBaseEngineSink::OnRequestWriteJoinInfo(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	...
	catch(...)
	{
		...
		ALErr("DBException GSP_GR_WritePersonalRoomJoinInfo");
	}
...
bool CDataBaseEngineSink::OnRequestWriteVideoPlayerInfo(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	...
	catch(...)
	{
		...
		ALErr("DBException GSP_GR_RecordVideoPlayerInfo");
	}
	return true;
}
...
bool CDataBaseEngineSink::OnRequestWriteVideoDataInfo(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	...
	catch(...)
	{
		...
		ALErr("DBException GSP_GR_RecordVideoDataInfo");
	}
	
	return true;
}

bool CDataBaseEngineSink::OnRequestGameConfigPulse(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID) {
	ALDeb("ReLoad misc game config...");
	// try {
	...
	// 	m_PlatformDBAide.AddParameterOutput(TEXT("@strErrorDescribe"), szDescribeString, sizeof(szDescribeString), adParamOutput);
	// 	//执行查询
	// 	LONG lResultCode = m_PlatformDBAide.ExecuteProcess(TEXT("GSP_GR_MiscGameConfig"),true);
