...
BEGIN
	...
	--	@CellOnlinePresent=CellOnlinePresent,@CellOnlineTime=CellOnlineTime,@StartOnlineTime=StartOnlineTime,
	--	@IsPlayPresent=IsPlayPresent,@IsOnlinePresent=IsOnlinePresent
	--	FROM WHJHPlatformDBLink.WHJHPlatformDB.dbo.GlobalPlayPresent WHERE ServerID=-1
	--	IF @ServerID IS NULL
	--	BEGIN
	--		SET @IsPlayPresent=0
 ...
 --   SELECT @MemberOrder=MemberOrder FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
	...
	DECLARE @EnterDiamond BIGINT
	DECLARE @DimandChange BIGINT
	
	SELECT @Dimand=Diamond FROM UserCurrency WHERE UserID=@dwUserID		
	IF @Dimand IS NULL SET @Dimand=0
	
	SELECT @EnterDiamond=EnterDiamond FROM RecordUserInout WHERE ID=@dwInoutIndex AND UserID=@dwUserID
	IF @EnterDiamond IS NULL SET @EnterDiamond=0
	
	SET @DimandChange=@Dimand-@EnterDiamond
	...
	UPDATE RecordUserInout SET LeaveTime=GetDate(), LeaveReason=@dwLeaveReason, LeaveMachine=@strMachineSerial,LeaveClientIP=@strClientIP,
		Score=@lRecordScore, Grade=@lRecordGrade, Insure=@lRecordInsure,Diamond=@DimandChange,Revenue=@lRecordRevenue, WinCount=@lRecordWinCount, LostCount=@lRecordLostCount,
	...
	-- 全局信息
	UPDATE WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo SET PlayTimeCount=PlayTimeCount+@dwRecordPlayTimeCount,OnLineTimeCount=OnLineTimeCount+@dwOnLineTimeCount
	WHERE UserID=@dwUserID
