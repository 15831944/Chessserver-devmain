...
BEGIN
	...
	SELECT @StartDateTime=StartDateTime,@LastDateTime=LastDateTime,@wSeriesDate=SeriesDate FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsSignin 	
	...
	BEGIN
		...
		INSERT INTO WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsSignin VALUES(@dwUserID,@StartDateTime,@LastDateTime,0)		
	END
	...
	BEGIN		
		...
		BEGIN
			SET @wSeriesDate = 0
			UPDATE WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsSignin SET StartDateTime=GetDate(),LastDateTime=GetDate(),SeriesDate=0 WHERE UserID=@dwUserID									
		END
	END
...
BEGIN
	...
	SELECT @LimitMachineCount=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'SignInLimitMachine'
	...
	BEGIN
		...
		SELECT @TodayMachineCount = COUNT(ClinetMachine) FROM WHJHRecordDB.dbo.RecordSignin WHERE DateDiff(d,CollectDate,GetDate())=0 AND @strMachineID = ClinetMachine
	...
	UPDATE WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsSignin SET LastDateTime = GetDate(),SeriesDate = @wSeriesDate WHERE UserID = @dwUserID

	-- 查询奖励
	DECLARE @lRewardGold BIGINT
	SELECT @lRewardGold=RewardGold FROM WHJHPlatformDBLink.WHJHPlatformDB.dbo.SigninConfig WHERE DayID=@wSeriesDate
	...
	BEGIN
		-- 插入资料
		INSERT INTO WHJHTreasureDBLink.WHJHTreasureDB.dbo.GameScoreInfo (UserID,Score, LastLogonIP, LastLogonMachine, RegisterIP, RegisterMachine)
		VALUES (@dwUserID, @lRewardGold, @strClientIP, @strMachineID, @strClientIP, @strMachineID)
	END
	

	-- 插入记录
	INSERT INTO WHJHRecordDB.dbo.RecordSignin(UserID,Gold,ClinetIP,ClinetMachine,CollectDate) VALUES (@dwUserID,@lRewardGold,@strClientIP,@strMachineID,GETDATE())	
	...
	UPDATE WHJHTreasureDBLink.WHJHTreasureDB.dbo.StreamPresentInfo SET DateID=@DateID, PresentCount=PresentCount+1,PresentScore=PresentScore+@lRewardGold WHERE UserID=@dwUserID AND TypeID=3
	IF @@ROWCOUNT=0
	BEGIN
		INSERT WHJHTreasureDBLink.WHJHTreasureDB.dbo.StreamPresentInfo(DateID,UserID,TypeID,PresentCount,PresentScore) VALUES(@DateID,@dwUserID,3,1,@lRewardGold)
	END	

	-- 查询金币
	DECLARE @lScore BIGINT	
	SELECT @lScore=Score FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.GameScoreInfo WHERE UserID = @dwUserID 	
