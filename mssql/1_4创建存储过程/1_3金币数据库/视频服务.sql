...
BEGIN
	...
	declare @Code	INT
	SET @Code=0      
	select @Code=PlayBackCode from  [WHJHTreasureDB].[dbo].[RecordVideoPlayerInfo] where VideoNumber=@szVideoNumber and UserID=@dwUserID and DateDiff(d,BuildVideoTime,GetDate()) <= 10
	...
	BEGIN
		...
		BEGIN
			...
			IF NOT EXISTS (select * from  [WHJHTreasureDB].[dbo].[RecordVideoPlayerInfo] where PlayBackCode=@Code and DateDiff(d,BuildVideoTime,GetDate()) <= 10)
			BEGIN
				SET @bFind = 1  
				UPDATE [WHJHTreasureDB].[dbo].[RecordVideoPlayerInfo] SET PlayBackCode =@Code WHERE UserID=@dwUserID and VideoNumber=@szVideoNumber   
			END
		END 
	END	
