...
bool __cdecl CGameVideoItemSink::AddVideoData(WORD wMsgKind,Video_GameStart *pVideoGameStart,bool bFirst)
{
	...
	WriteUint32(pVideoGameStart->lServiceCharge);

	WriteUint64(pVideoGameStart->lPersonalInitScore);
	WriteUint8(pVideoGameStart->cbPersonalGameCount);
	WriteUint8(pVideoGameStart->cbPersonalPlayerMode);

	WriteUint64(pVideoGameStart->lDrawMaxScore);
	WriteUint64(pVideoGameStart->lTurnMaxScore);
	WriteUint64(pVideoGameStart->lTurnLessScore);

	WriteUint16(pVideoGameStart->wCurrentUser);

	WriteUint8(pVideoGameStart->cbObscureCard);
	Write(pVideoGameStart->cbCardData,sizeof(BYTE)*GAME_PLAYER);
	Write(pVideoGameStart->cbHandCardData,sizeof(BYTE)*GAME_PLAYER*5);
...
bool __cdecl CGameVideoItemSink::AddVideoData(WORD wMsgKind,CMD_S_GiveUp *pVideoGiveUp)
{
	...
	WriteUint16(pVideoGiveUp->wCurrentUser);
	WriteUint64(pVideoGiveUp->lDrawMaxScore);
	WriteUint64(pVideoGiveUp->lTrunMaxScore);
...
bool __cdecl CGameVideoItemSink::AddVideoData(WORD wMsgKind,CMD_S_AddScore *pVideoAddScore)
{
	//一定要按顺序写
	WriteUint16(wMsgKind);
	WriteUint16(pVideoAddScore->wCurrentUser);
	WriteUint16(pVideoAddScore->wAddScoreUser);
	WriteUint64(pVideoAddScore->lTurnLessScore);
	WriteUint64(pVideoAddScore->lUserScoreCount);
...
bool __cdecl CGameVideoItemSink::AddVideoData(WORD wMsgKind,CMD_S_SendCard *pVideoSendCard)
{
	...
	WriteUint16(pVideoSendCard->wStartChairID);
	WriteUint64(pVideoSendCard->lTurnMaxScore);

	WriteUint8(pVideoSendCard->cbSendCardCount);
	Write(pVideoSendCard->cbCardData,sizeof(BYTE)*2*GAME_PLAYER);
...
bool __cdecl  CGameVideoItemSink::AddVideoData(WORD wMsgKind,CMD_S_GameEnd *pVideoGameEnd)
{
	...
	Write(pVideoGameEnd->cbCardData,sizeof(BYTE)*GAME_PLAYER);
	Write(pVideoGameEnd->lGameScore,sizeof(LONGLONG)*GAME_PLAYER);
	WriteUint64(pVideoGameEnd->lMinTableScore);
