...
protected:
	...
	//手机验证码
	bool OnTCPNetworkSubMBRegisterCode(int kind, VOID * pData, WORD wDataSize, DWORD dwSocketID);
	//重置密码
	bool OnTCPNetworkSubMBResetPasswd(VOID * pData, WORD wDataSize, DWORD dwSocketID);
	//游客登录
...
//////////////////////////////////////////////////////////////////////////////////
#define AT_PROTO_REPRET(sock,code) \
qpcmd::RepRet rep; \
rep.set_code(code); \
rep.set_error(RrpcErrorString(code)); \
std::string repData; \
rep.SerializeToString(&repData); \
m_pITCPNetworkEngine->SendData(sock,MDM_MB_LOGON,SUB_MB_LOGON_REPRET,(void*)repData.data(),repData.size()); \
m_pITCPNetworkEngine->ShutDownSocket(sock)
