...
#include "game.rawrpc.h"
#include "../utility/AsyncLog.h"
#include "../msgdef/CMD_LKFish.h"
	...
	if (m_bGameStarted==false)
	{
		for (WORD i=0;i<m_wChairCount;i++)
		{
				...
				)
			{
				...
				{
					_sntprintf_s(szDescribe,CountArray(szDescribe),TEXT("您的游戏币少于 ") SCORE_DISPLAY TEXT("，不能继续游戏！"),
						m_pGameServiceOption->lMinTableScore*1.0/1000);
				}
				else
				{
					_sntprintf_s(szDescribe,CountArray(szDescribe),TEXT("您的游戏积分少于 ") SCORE_DISPLAY TEXT("，不能继续游戏！"),
						m_pGameServiceOption->lMinTableScore*1.0/1000);
				}
			...
			if (((m_pGameServiceOption->lMinTableScore!=0L) && pIServerUserItem->GetUserScore()<m_pGameServiceOption->lMinTableScore * m_lCellScore/m_pGameServiceOption->lCellScore) && ((m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) != 0))
			{
				...
				{
					_sntprintf_s(personalRoomMessage.szMessage,CountArray(personalRoomMessage.szMessage),TEXT("您的游戏币少于 ") SCORE_DISPLAY TEXT("，不能继续游戏！"),
						(m_pGameServiceOption->lMinTableScore* m_lCellScore/m_pGameServiceOption->lCellScore)*1.0/1000);
...
{
	// 发送玩家赢得金币喇叭
	if (wSubCmdID == SUB_S_WIN_TRUMPET) {
		if (!pIServerUserItem || !m_pIMainServiceFrame) return true;
		CMD_GC_GTrumpet* pTrumpet = (CMD_GC_GTrumpet*)pData;
		RPCGAME(rpc);
		if ( (!pIServerUserItem->IsAndroidUser() && pTrumpet->lScore >= rpc.mTrumpetScore) ||
				pTrumpet->lScore >= (rpc.mTrumpetScore*10) ) {
			pTrumpet->wGame = m_pGameServiceOption->wKindID;
			pTrumpet->lCellScore = m_pGameServiceOption->lCellScore;
			lstrcpyn(pTrumpet->szNickName, pIServerUserItem->GetNickName(), 31);
			//lstrcpyn(pTrumpet->szRoomName, m_pGameServiceOption->szServerName, 31);
			try {
				CAttemperEngineSink* pSink = dynamic_cast<CAttemperEngineSink*>(m_pIMainServiceFrame);
				if (pSink) pSink->PostServiceData(SUB_CS_C_PROPERTY_TRUMPET, pData, wDataSize);
			} catch (...) {
				ALErr("not available main service frame %p", m_pIMainServiceFrame);
			}
		}
		return true;
	} else if (wSubCmdID == SUB_S_AWARD_TIP && sizeof(CMD_S_AwardTip) == wDataSize) {
		// 捕鱼游戏喇叭
		CMD_S_AwardTip* pAward = (CMD_S_AwardTip*)pData;
		RPCGAME(rpc);
		ALDeb("Fish turmpet %p %d - %ld", m_pIMainServiceFrame, rpc.mTrumpetScore, pAward->lFishScore);
		if (m_pIMainServiceFrame && pAward->lFishScore >= rpc.mTrumpetScore) {
			CMD_GC_GTrumpet sTrumpet;
			memset(&sTrumpet, 0, sizeof(CMD_GC_GTrumpet));
			sTrumpet.wGame = m_pGameServiceOption->wKindID;
			sTrumpet.wMultiple = pAward->nFishMultiple;
			sTrumpet.dwCardType = pAward->nFishType;
			sTrumpet.lScore = pAward->lFishScore;
			sTrumpet.lCellScore = m_pGameServiceOption->lCellScore;
			lstrcpyn(sTrumpet.szNickName, pAward->szPlayName, 31);
			//lstrcpyn(sTrumpet.szRoomName, m_pGameServiceOption->szServerName, 31);
			try {
				CAttemperEngineSink* pSink = dynamic_cast<CAttemperEngineSink*>(m_pIMainServiceFrame);
				if (pSink) pSink->PostServiceData(SUB_CS_C_PROPERTY_TRUMPET, &sTrumpet, sizeof(CMD_GC_GTrumpet));
			} catch (...) {
				ALErr("not available main service frame %p", m_pIMainServiceFrame);
			}
		}
...
{
	if (wType == SMT_GAMELOG && lpszMessage) {
		UNI2UTF_S(lpszMessage, pMsg);
		ALStd("%s", pMsg);
		return true;
	}
	...
	case SUB_GF_GAME_OPTION:	//游戏配置
		{
			...
			if (((m_pGameServiceOption->lMinTableScore!=0L) && pIServerUserItem->GetUserScore()<m_pGameServiceOption->lMinTableScore * m_lCellScore/m_pGameServiceOption->lCellScore) && ((m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) != 0))
			{
				...
				if (lstrcmp(m_pGameServiceOption->szDataBaseName, szTreasureDB) == 0)
				{
					_sntprintf(personalRoomMessage.szMessage,CountArray(personalRoomMessage.szMessage),TEXT("您的游戏币少于 ") SCORE_DISPLAY TEXT("，不能继续游戏！"),
						(m_pGameServiceOption->lMinTableScore* m_lCellScore/m_pGameServiceOption->lCellScore)*1.0/1000);
...
bool CTableFrame::PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem, LPCTSTR lpszPassword,DWORD dwRoomPwd)
{
	...
	if(((m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) != 0) && m_cbPayMode != 0)
	{
		...
		{
			Personal_Room_Message personalRoomMessage;
			_sntprintf_s(personalRoomMessage.szMessage,CountArray(personalRoomMessage.szMessage),TEXT("您的钻石数量少于 ") SCORE_DISPLAY TEXT("，不能继续游戏！"),
				(m_lPayRoomCardCount/m_PersonalTableParameter.wJoinGamePeopleCount)*1.0/1000);
			personalRoomMessage.cbMessageType = 0;
			m_pIMainServiceFrame->SendData(pIServerUserItem,MDM_GR_PERSONAL_TABLE,SUB_GF_PERSONAL_MESSAGE,&personalRoomMessage,sizeof(personalRoomMessage));
			return false;
		}
...
bool CTableFrame::EfficacyEnterTableScoreRule(WORD wChairID, IServerUserItem * pIServerUserItem)
{
	...
	{
		//构造信息
		double displayScore = __max(lMinTableScore,lLessEnterScore)*1.0/1000;
		TCHAR szDescribe[128]=TEXT("");
		if(m_pGameServiceOption->wServerType==GAME_GENRE_GOLD)
			_sntprintf_s(szDescribe,CountArray(szDescribe),TEXT("加入游戏至少需要 ") SCORE_DISPLAY TEXT(" 的游戏币，您的游戏币不够，不能加入！"),displayScore);
		else if(m_pGameServiceOption->wServerType==GAME_GENRE_MATCH)
			_sntprintf_s(szDescribe,CountArray(szDescribe),TEXT("加入游戏至少需要 ") SCORE_DISPLAY TEXT(" 的比赛币，您的比赛币不够，不能加入！"),displayScore);
		else
		{
			if (m_pGameServiceOption->wServerType != GAME_GENRE_PERSONAL)
			{
				_sntprintf_s(szDescribe,CountArray(szDescribe),TEXT("加入游戏至少需要 ") SCORE_DISPLAY TEXT(" 的游戏积分，您的积分不够，不能加入！"),displayScore);
...
bool CTableFrame::CancelTableRequestReply(DWORD dwUserID, BYTE cbAgree)
{
	...
	if (nCancelCount > ((m_nPersonalRoomPlayerCount) / 2))
	{
		m_cbIsRequestOver = 0;
	...
	//所有玩家都处理了，关闭解散超时定时器
	if (nDealCancelCount == m_nPersonalRoomPlayerCount)
	{
		KillGameTimer(IDI_TIME_CANCEL);
	}

	if(m_dwReplyCount < wOnlineCount && m_cbRequestReply[wChairID] == 2 ) return true;
	if (m_dwReplyCount < wOnlineCount && wAgreeCount <  ((m_nPersonalRoomPlayerCount) / 2 + 1) && m_nPersonalRoomPlayerCount != 2)
	{
		return true;
	}

	if (wAgreeCount >= wOnlineCount || wAgreeCount >= ((m_nPersonalRoomPlayerCount) / 2 + 1) && m_nPersonalRoomPlayerCount != 2)
	{
		bCancel = true;
	}
