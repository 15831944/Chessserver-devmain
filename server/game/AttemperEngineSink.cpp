...
bool CAttemperEngineSink::OnAttemperEngineStart(IUnknownEx * pIUnknownEx)
{
	...
	//设置时间
	m_pITimerEngine->SetTimer(IDI_GAME_SERVICE_PULSE,60*1000L,TIMES_INFINITY,NULL);
	...
	try
	{
		...
		if ((dwTimerID>=IDI_MAIN_MODULE_START)&&(dwTimerID<=IDI_MAIN_MODULE_FINISH))
		{
			...
			case IDI_LOAD_ANDROID_USER:		//加载机器
				{
					...
					if(m_AndroidUserManager.GetAndroidLoadInfo(dwBatchID,dwAndroidCount))
					{
						...
						ALStd("Post Load android batch %d count %d", dwBatchID, dwAndroidCount);
					}
			...
			case IDI_GAME_SERVICE_PULSE:	//服务维护
				{
					RPCGAME(rpc);
					rpc.UpdateTrumpetScore();
					ALStd("trumpet score %d", rpc.mTrumpetScore);
					return true;
				}
...
bool CAttemperEngineSink::OnEventTCPSocketLink(WORD wServiceID, INT nErrorCode)
{
	...
	if (wServiceID==NETWORK_CORRESPOND)
	{
		...
		//构造数据
		ALDeb("Reg room cell(%d), enter(%d)", m_pGameServiceOption->lCellScore, m_pGameServiceOption->lMinEnterScore);
...
{
	//协调连接
	ALStd("Service Socket[%d] Read %d-%d", wServiceID, Command.wMainCmdID, Command.wSubCmdID);
	if (wServiceID==NETWORK_CORRESPOND)
	{
...
bool CAttemperEngineSink::OnEventTCPNetworkBind(DWORD dwClientAddr, DWORD dwSocketID)
{
	...
	ALStd("Client in %d-%d", wBindIndex, dwSocketID);
	//设置变量
...
bool CAttemperEngineSink::OnEventTCPNetworkShut(DWORD dwClientAddr, DWORD dwActiveTime, DWORD dwSocketID)
{
	...
	WORD wBindIndex=LOWORD(dwSocketID);
	tagBindParameter * pBindParameter=GetBindParameter(wBindIndex);
	ALStd("Client out %d-%d", wBindIndex, dwSocketID);
...
bool CAttemperEngineSink::OnEventTCPNetworkRead(TCP_Command Command, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	if (MDM_GF_GAME != Command.wMainCmdID) {
		ALStd("Client[%d] cmd %d-%d", dwSocketID, Command.wMainCmdID, Command.wSubCmdID);
	}
	switch (Command.wMainCmdID)
	{
...
bool CAttemperEngineSink::OnEventUserItemScore(IServerUserItem * pIServerUserItem, BYTE cbReason)
{
		...
		&& pIServerUserItem->IsVariation()==true)
	{
		...
		//RPC statistic
		if (WriteGameScore.VariationInfo.dwPlayTimeCount > 0 || WriteGameScore.VariationInfo.lScore > 0) {
			UNI2UTF_S(pUserInfo->szNickName, pNick);
			RPCGAME(rpc);
			rpc.GamePlayStatis(pUserInfo->dwUserID, m_pGameServiceAttrib->wKindID, pUserInfo->wFaceID,
				WriteGameScore.VariationInfo.dwPlayTimeCount, WriteGameScore.VariationInfo.lScore, pNick);
		}
		//调整分数
		if(pIServerUserItem->IsAndroidUser()==true)
		{
...
bool CAttemperEngineSink::OnDBLogonSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	//最低分数
	if ((m_pGameServiceOption->lMinEnterScore!=0L)&&
		(pDBOLogonSuccess->lScore<m_pGameServiceOption->lMinEnterScore && pDBOLogonSuccess->cbMasterOrder==0))
	{
		//发送失败
		TCHAR szMsg[128]=TEXT("");
		_sntprintf_s(szMsg,CountArray(szMsg), TEXT("抱歉，您的金币低于") SCORE_DISPLAY TEXT("，不能进入当前游戏房间！"),
			m_pGameServiceOption->lMinEnterScore*1.0/1000);
	...
	//最高分数
	if ((m_pGameServiceOption->lMaxEnterScore!=0L)&&
		(pDBOLogonSuccess->lScore>m_pGameServiceOption->lMaxEnterScore && pDBOLogonSuccess->cbMasterOrder==0))
	{
		//发送失败
		TCHAR szMsg[128]=TEXT("");
		_sntprintf_s(szMsg,CountArray(szMsg), TEXT("抱歉，您的金币高于") SCORE_DISPLAY TEXT("，不能进入当前游戏房间！"),
			m_pGameServiceOption->lMaxEnterScore*1.0/1000);
		SendLogonFailure(szMsg,0,pBindParameter->dwSocketID);
...
bool CAttemperEngineSink::OnTCPNetworkSubLogonUserID(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	ALDeb("%d id logon", pLogonUserID->dwUserID);
	return true;
}
...
bool CAttemperEngineSink::OnTCPNetworkSubLogonMobile(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	UNI2UTF_S(pLogonMobile->szPassword, pPass);
	ALDeb("%d UID %d pass %s", dwSocketID, pLogonMobile->dwUserID, pPass);
...
bool CAttemperEngineSink::OnTCPNetworkSubSaveScoreRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	if (pSaveScoreRequest->lSaveScore>lConsumeQuota)
	{
		if (lConsumeQuota<lUserWholeScore)
		{
			//构造提示
			TCHAR szDescribe[128]=TEXT("");
			_sntprintf_s(szDescribe,CountArray(szDescribe),TEXT("由于您正在游戏中，游戏币可存入额度为") SCORE_DISPLAY TEXT("，存入操作失败！"),
				lConsumeQuota*1.0/1000);
...
{
	ALDeb("Unlock server %d batch %d android", wServerID, wBatchID);
	//解锁机器
