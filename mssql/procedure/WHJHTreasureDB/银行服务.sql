...
BEGIN
	...
	-- 查询用户
	IF NOT EXISTS(SELECT * FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID AND LogonPass=@strLogonPass)
	BEGIN
	...
	-- 设置密码
	UPDATE WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo SET InsurePass=@strInsurePass	WHERE UserID=@dwUserID		
...
BEGIN
	...
	-- 系统暂停
	SELECT @EnjoinInsure=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'EnjoinInsure'
	IF @EnjoinInsure IS NOT NULL AND @EnjoinInsure<>0
	BEGIN
		SELECT @strErrorDescribe=StatusString FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'EnjoinInsure'
		RETURN 2
	END
	...
	FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
	...
	SELECT @BankPrerequisite=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'BankPrerequisite'
	IF @BankPrerequisite IS NULL SET @BankPrerequisite=0
	IF @lSaveScore<@BankPrerequisite
	BEGIN
		SET @strErrorDescribe=N'存入银行的游戏币数目不能少于 '+LTRIM(STR(@BankPrerequisite/1000))+'，游戏币存入失败！'
		RETURN 4
	END
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @KindName=KindName FROM WHJHPlatformDBLink.WHJHPlatformDB.dbo.GameKindItem WHERE KindID=@LockKindID
...
BEGIN
	...
	BEGIN
		SET @strErrorDescribe=N'从银行取出的游戏币数目不能少于 '+LTRIM(STR(@BankPrerequisite/1000))+'，游戏币提取失败！'
		RETURN 4
	END

	-- 银行税收
	DECLARE @RevenueRate INT
	SELECT @RevenueRate=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'RevenueRateTake'
	...
	DECLARE @SerialNumber NVARCHAR(20)
	SET @DateTime = GETDATE()
	IF @InsureRevenue>0 -- 取款税收 存在时 记入金币流水 为 银行服务费
	BEGIN 
		SELECT @SerialNumber=dbo.WF_GetSerialNumber()
		INSERT INTO WHJHRecordDB.dbo.RecordTreasureSerial(SerialNumber,MasterID,UserID,TypeID,CurScore,CurInsureScore,ChangeScore,ClientIP,CollectDate) 
	...
	SELECT @SerialNumber=dbo.WF_GetSerialNumber()
	INSERT INTO WHJHRecordDB.dbo.RecordTreasureSerial(SerialNumber,MasterID,UserID,TypeID,CurScore,CurInsureScore,ChangeScore,ClientIP,CollectDate) 
	VALUES(@SerialNumber,0,@dwUserID,7,@SourceScore,@SourceInsure-@InsureRevenue,@VariationScore,@strClientIP,DATEADD(ms,10,@DateTime))	
...
BEGIN
	...
	SELECT @TUserRight=UserRight FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
	SET @UserOpenTransfer = @TUserRight&0x00000040	
	IF @UserOpenTransfer=0
	BEGIN
		SELECT @EnjoinTransfer=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'TransferStauts'
		IF @EnjoinTransfer IS NULL SET @EnjoinTransfer=0
		IF @EnjoinTransfer=1 
		BEGIN
			SELECT @strErrorDescribe=StatusString FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'TransferStauts'
			RETURN 3
		END	
	END
	-- 效验地址
	...
	FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID and IsAndroid=0
	...
	SELECT @TransferPrerequisite=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'TransferPrerequisite'
	IF @TransferPrerequisite IS NULL SET @TransferPrerequisite=0
	IF @lTransferScore<@TransferPrerequisite
	BEGIN
		SET @strErrorDescribe=N'从银行转账的游戏币数目不能少于 '+LTRIM(STR(@TransferPrerequisite/1000))+'，游戏币转账失败！'
		RETURN 4
	END
	...
	SELECT @TargetUserID=UserID,@UserRight=UserRight,@DstMemberOverDate =MemberOverDate,@DstMemberOrder=MemberOrder FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo WHERE GameID=@dwGameID	
--	IF @cbByNickName=1
--		SELECT @TargetUserID=UserID FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo WHERE NickName=@strNickName
--	ELSE
--		SELECT @TargetUserID=UserID FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo WHERE GameID=@strNickName	
	...
	SELECT @SystemTransferRight=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'TransferPower'
	...
	IF @MemberOrder = 0
	BEGIN
		SELECT @MaxTax=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'TransferMaxTax'
		SELECT @RevenueRate=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'RevenueRateTransfer'
	...
	BEGIN
		SELECT @MaxTax=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'TransferMaxTax'
		--SELECT @RevenueRate=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'RevenueRateTransferMember'		
			SELECT @RevenueRate=InsureRate FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.MemberProperty where @MemberOrder =MemberOrder
		-- 税收调整
	...
	SELECT @TransferRetention=ISNULL(StatusValue,0) FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'TransferRetention'
	...
	BEGIN
		...
		BEGIN
			...
			SET @strErrorDescribe=N'非常抱歉,转账后您银行的余额不能少于'+LTRIM(@TransferRetention/1000)+'金币'
			RETURN 7
		END
	END
	...
	-- 设置信息
	SET @strErrorDescribe=N'转账 '+LTRIM(STR(@lTransferScore/1000))+' 游戏币到 '+LTRIM(@dwGameID)+'的银行操作成功，请查验您的帐户信息！'
...
BEGIN
	...
	SELECT @LogonPass=LogonPass FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
	...
	SELECT @RevenueRateTake=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'RevenueRateTake'
	SELECT @RevenueRateTransfer=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'RevenueRateTransfer'
	SELECT @RevenueRateTransferMember=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'RevenueRateTransferMember'
	SELECT @TransferPrerequisite=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'TransferPrerequisite'
	SELECT @EnjoinTransfer=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'TransferStauts'
	...
	--查询权限
	SELECT @UserRight=UserRight FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
	...
	SELECT @dwUserID AS UserID, @Score AS Score, @Insure AS Insure, @ServerID AS ServerID, @RevenueRateTake AS RevenueTake, 
		   ...
		   @EnjoinTransfer AS EnjoinTransfer,@UserRight AS UserRight
...
BEGIN
	...
	SELECT @cbRebateEnabled=StatusValue FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'TransferRebate'
