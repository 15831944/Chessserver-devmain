...
BEGIN
	...
	IF NOT EXISTS ( SELECT * FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID AND IsAndroid =1)
	BEGIN
		SELECT @strPassword=DynamicPass FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
